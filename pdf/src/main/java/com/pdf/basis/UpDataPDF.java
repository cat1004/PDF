...
/**
 * @author : chenpeng
 * @date : 2018-08-07 09:54 在原有pdf 上修改pdf 文件中的信息 适用于签章,标注,水印等操作.
 */
public class UpDataPDF {
  public static void main(String[] args) throws IOException, DocumentException {
    String tmp = "";
    UpDataPDF.fileChannelCopy(
        new File("D:\\temp\\myFirstPDF.pdf"), new File("D:\\temp\\myFirstPDF2.pdf"));
  ...
  public static void fileChannelCopy(File sources, File dest) {
    try {
      ...
      fileChannelin.transferTo(
          0, fileChannelin.size(), fileChannelout); // 连接两个通道，并且从in通道读取，然后写入out通道
