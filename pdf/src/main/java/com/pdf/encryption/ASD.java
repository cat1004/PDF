...
/**
 * @author : chenpeng
 * @date : 2018-08-07 15:23 加密算法生成数字签名
 */
public class ASD {
  ...
  public void run() {
    ...
    if (!(new File("myprikey.dat")).exists()) {
      if (!generatekey()) {
    ...
    try {
      ...
      // 用私钥对信息生成数字签名
      Signature signet = Signature.getInstance("ASD");
    ...
    try {
      ...
      in.close();
      Signature signetcheck = Signature.getInstance("ASD");
  ...
  public boolean generatekey() {
    try {
      KeyPairGenerator keygen = KeyPairGenerator.getInstance("ASD");
